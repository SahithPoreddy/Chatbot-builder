...
import DiagramJsonButtons from '~/modules/sidebar/components/diagram-json-buttons';
import DiagramSvgButton from '~/modules/sidebar/components/diagram-svg-button';
// import UndoRedoButtons from '~/modules/sidebar/components/undo-redo-buttons';
...
export function DesktopSidebarFragment({ activePanel, setActivePanel }: DesktopSidebarFragmentProps) {
  const { getNodes, getEdges, setNodes, setEdges, exportAsSvg } = useApplicationState(s => s.diagram);

  // Only render buttons if functions are available
  const renderButtons = getNodes && getEdges && setNodes && setEdges;

  // Removed auto-open effect to allow toggle functionality
