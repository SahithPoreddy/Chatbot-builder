...
//     return cases.filter(c => !data.paths.some(p => p.case.value === c.value))
//   }, [cases, data.paths])
...
import ConditionalPathNodePropertyPanel from '~/modules/sidebar/panels/node-properties/property-panels/conditional-path-property-panel';
...
export function ConditionalPathNode({ id, isConnectable, selected, data }: ConditionalPathNodeProps) {
  ...
  const deleteNode = useDeleteNode();
  const { cases } = useCaseStore();
  const [setActivePanel, showNodePropertiesOf] = useApplicationState((s: any) => [s.actions.sidebar.setActivePanel, s.actions.sidebar.showNodePropertiesOf]);

  const handleDoubleClick = useCallback(() => {
    setActivePanel('node-properties');
    showNodePropertiesOf({ id, type: BuilderNode.CONDITIONAL_PATH });
  }, [id, setActivePanel, showNodePropertiesOf]);
...
export const metadata: RegisterNodeMetadata<ConditionalPathNodeData> = {
  type: NODE_TYPE,
  node: memo(ConditionalPathNode),
  propertyPanel: ConditionalPathNodePropertyPanel,
  detail: {
    icon: 'i-mynaui:git-branch',
    title: 'Conditional Path',
    description: 'Check a condition and take different paths based on the result.',
